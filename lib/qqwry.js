...
var ipdbPathDefined = path.join(__dirname, "../data/qqwry.ipdb"); //ipdb默认路径
var dbug = false;
...
function ipdbPack() {
  ...
  function IP(ip, language) {
    switch (arguments.length) {
      ...
      case 2:
      default:
        return self.searchIP(ip, language);
    }
  }
  IP._lib = self;
  Object.keys(IpdbDriver.prototype).forEach(function (key) {
    if (typeof self[key] == "function") {
      IP[key] = self[key].bind(self);
    }
  });
...
function getCmd(isspeed, path) {
  return isspeed ? bufferCmd(path) : fileCmd(path);
}
...
function QqwryDriver(speed, dataPath) {
  ...
  this.cmd = getCmd(isspeed, this.dataPath);
  var cmd = this.cmd();
...
QqwryDriver.prototype.speed = function () {
  ...
  this.cmd = getCmd(true, this.dataPath);
  return this;
};
...
QqwryDriver.prototype.unSpeed = function () {
  ...
  this.cmd = getCmd(false, this.dataPath);
  return this;
};
...
function LocateIP(ip) {
  ...
  if (dbug) {
    var begip = cmd.readUIntLE(g, 4);
    var endip = cmd.readUIntLE(cmd.readUIntLE(g + 4, 3), 4); //获取结束IP的值
    log(intToIP(ip) + " >> " + ip);
 ...
 * @param {string} [options.language="CN"] 查询语言
 */
function IpdbDriver(dataPath, options) {
  if (!(this instanceof IpdbDriver)) {
    return new IpdbDriver(dataPath, options);
  }
  options = options || {};
  this.dataPath = dataPath || ipdbPathDefined;
  this.language = options.language || "CN";
  this.cmdFactory = ipdbCmd(this.dataPath);
  return ipdbPack.call(this);
}

/**
 * 单IP查询
 * @param {string|number} IP IP地址
 * @param {string} [language] 查询语言
 * @return {object}
 */
IpdbDriver.prototype.searchIP = function (IP, language) {
  var lang = language || this.language;
  var cmd = this.cmdFactory();
  var ip = typeof IP === "number" ? intToIP(IP) : IP;
  var data = cmd.find(ip, lang);
 ...
 * @return {string[]}
 */
IpdbDriver.prototype.fields = function () {
 ...
 * @return {string[]}
 */
IpdbDriver.prototype.languages = function () {
 ...
 * @return {number}
 */
IpdbDriver.prototype.buildTime = function () {
 ...
 * 极速模式 (ipdb始终为内存模式)
 */
IpdbDriver.prototype.speed = function () {
  return this;
};
 ...
 * 关闭极速模式 (ipdb始终为内存模式)
 */
IpdbDriver.prototype.unSpeed = function () {
  return this;
};
 ...
 * @return {IpdbDriver}
 */
QqwryDriver.ipdb = function (dataPath, options) {
  return new IpdbDriver(dataPath, options);
};
...
module.exports.IpdbDriver = IpdbDriver;
